...
class CreateRideRatingSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        ...
        ride=self.context['ride'] # O tambien podiamos colocar self.context['view'].get_object(), en caso de que no lo coloquemos al contexto
        if not ride.passengers.filter(pk=user.pk).exists(): # Vericamos que solo los pasajeros del viaje puedan califcar el viaje
    ...
    def create(self,data):
        ...
        ride_avg=round(
            Rating.objects.filter(
                circle=self.context['circle'],
                ride=self.context['ride']
            ).aggregate(Avg('rating'))['rating__avg'], # Sacamos el promedio y guardamos el valor de la llave  rating_avg que es el promedio. 
