...
class InvitationsManagerTestCase(TestCase):
    ...
    def test_code_usage(self):
        ...
        self.assertEqual(invitation.code, code)  # Si el codigo que colocamos no es igual al que deberia estar
    # en la instacia de invitations del campo code, django nos mostrara en donde fallo al ejecutar las pruebas.
    ...
    def test_code_generation_if_duplicated(self):
        ...
        self.assertNotEqual(code, invitation.code)  # Verificamos que el codigo nuevo no sea el mismo que
        # creamos anteriormente, tiene que generar un nuevo.
...
class MemberInvitationsAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION='Token {}'.format(self.token))  # Mandamos al atributo
        # "client" la autorizacion del token
    ...
    def test_response_success(self):
        ...
        request = self.client.get(self.url)  # self.client.<metodo>() es un atributo que viene incorporado
        # desde APIClient. El request no solo enviara los datos sino que tambien traera los datos
        # de la respuesta.
    ...
    def test_invitation_creation(self):
        ...
        self.assertEqual(Invitation.objects.count(), 0)  # Traemos el numero de registros de Invitaciones que
        # existen de prueba.Por defecto tienen estar vacio
        ...
        self.assertEqual(invitations.count(), self.membership.remaining_invitations)  # Comparamos que el
        # numero de datos encontrados sea igual al numero de invitaciones que le quedan al usuario.
